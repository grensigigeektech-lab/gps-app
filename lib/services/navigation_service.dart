...
class NavigationService {
  ...
  Future<NavigationRouteResult> getDrivingRoute({
    ...
    _ensureConfigured();
    _validateCoordinate(origin, isOrigin: true);
    _validateCoordinate(destination, isOrigin: false);
    ...
    for (final rawCoordinate in rawCoordinates) {
      if (rawCoordinate is! List || rawCoordinate.length < 2) {
        throw const NavigationException(
          NavigationFailureType.noRoute,
          'The route contained invalid map coordinates. Please retry.',
        );
      }
  ...
  Future<Map<String, dynamic>> _getJson(Uri uri) async {
    ...
    if (response.statusCode == 422 &&
        uri.path.contains('/directions/') &&
        _isNoRouteResponse(response.body)) {
      throw const NavigationException(
        NavigationFailureType.noRoute,
        'No drivable route could be generated for this destination.',
      );
    ...
    if ((response.statusCode == 400 || response.statusCode == 422) &&
        uri.path.contains('/geocoding/')) {
      throw const NavigationException(
        NavigationFailureType.invalidDestination,
        'The destination could not be searched. Try a full address or place name.',
      );
    }
  ...
  void _validateCoordinate(
    ...
    if (coordinate.latitude.isFinite &&
        ...
        coordinate.latitude.abs() <= 90 &&
        coordinate.longitude.abs() <= 180) {
      return;
    }
    ...
    throw NavigationException(
      isOrigin
          ...
          : NavigationFailureType.invalidDestination,
      isOrigin
          ? 'Your current GPS location is invalid. Refresh your location and retry.'
          : 'The destination coordinates are invalid. Try another address.',
    );
  }

  bool _isNoRouteResponse(String body) {
    try {
      final decoded = jsonDecode(body);
      if (decoded is! Map<String, dynamic>) return false;
      return decoded['code'] == 'NoRoute' || decoded['code'] == 'NoSegment';
    } on FormatException {
      return false;
    }
  }
