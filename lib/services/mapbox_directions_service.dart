...
class MapboxDirectionsService {
  ...
  static String? validateDestination(String input) {
    ...
    if (text.runes.length > 256) {
      return 'Keep the destination under 257 characters.';
    }
    if (text.contains(';')) {
      return 'Use commas instead of semicolons in the destination.';
    }
    if (RegExp(r'[\p{L}\p{M}\p{N}]+', unicode: true).allMatches(text).length >
        20) {
      return 'Use a shorter destination, with no more than 20 words.';
    }
    if (text.length < 2 ||
  ...
  Future<List<NavigationDestination>> searchDestinations(
    ...
    try {
      ...
      return features.map(_parseDestination).toList(growable: false);
    } on FormatException {
      throw _invalidResponse;
    }
  }

  /// Used by the existing camera's optional address overlay as well as search.
  Future<String?> reverseGeocode(RouteCoordinate coordinate) async {
    if (!coordinate.isValid) throw _invalidResponse;
    final data = await _get('/search/geocode/v6/reverse', {
      'longitude': '${coordinate.longitude}',
      'latitude': '${coordinate.latitude}',
      // Do not set limit: v6 permits it only with exactly one feature type.
      // The default returns the most specific address, then broader places.
    });
    try {
      final features = data['features'];
      if (features is! List) throw const FormatException();
      return features.isEmpty ? null : _parseDestination(features.first).name;
    } on FormatException {
      throw _invalidResponse;
    }
  }

  static NavigationDestination _parseDestination(Object? feature) {
    if (feature is! Map) throw const FormatException();
    final properties = feature['properties'];
    final geometry = feature['geometry'];
    if (properties is! Map || geometry is! Map || geometry['type'] != 'Point') {
