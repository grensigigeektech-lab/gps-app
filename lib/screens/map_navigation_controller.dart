...
class MapNavigationController extends GetxController
  ...
  Future<void> refreshLocation() async {
    ...
    _resetResult();
    if (await _getLocation(request) && _active(request)) {
      phase.value = NavigationPhase.idle;
  ...
  Future<void> search() async {
    ...
    if (!await _getLocation(request) || !_active(request)) return;
    phase.value = NavigationPhase.searching;
  ...
  void onMapLayout(double width, double height) {
    if (_closed ||
        width <= 0 ||
        height <= 0 ||
    ...
    _mapWidth = width;
    _mapHeight = height;
    // Route summaries, error messages, accessibility text and the keyboard can
    // resize the native view. Fit after Flutter lays out the new map bounds.
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (!_closed && _mapReady) unawaited(fitRoute());
    });
  }
